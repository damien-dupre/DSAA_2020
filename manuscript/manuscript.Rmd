...
house_dublin_2018 <- geocode_dublin_2018 %>% 
  dplyr::filter(!grepl('apartment|Apartment|APART|APT|Apt', Address))

final_dublin_2018 <- here::here("data/data_dublin_2018_osm_features.rds") %>% 
  readr::read_rds()
...
Because of the evolution of urban planning and the evolution of socio-economic features measured by the 2016 census, only properties sold in 2018 were processed to avoid potential temporal incoherence. The total amount of property sold in 2018 in the county Dublin which appears in the Property Price Register is `r nrow(geocode_dublin_2018)`. From these, all property corresponding to apartments were removed (`r scales::percent(1-(nrow(house_dublin_2018)/nrow(geocode_dublin_2018)), accuracy = 0.1)` of the original dataset). Finally, unsuccessfully geocoded properties were also removed from the original dataset (`r scales::percent(1-(nrow(final_dublin_2018)/nrow(house_dublin_2018)), accuracy = 0.1)`) to obtain the `r nrow(final_dublin_2018)` of the final dataset used to identify the urban and socio-economic correlates of property prices.
...
In 2018, the average price of a sold property in Dublin was `r scales::dollar(round(mean_dat, 0), suffix = "", prefix = "€")` with a standard deviation of `r scales::dollar(round(sd_dat, 0), suffix = "", prefix = "€")`. In order to remove potential human errors and outliers, prices higher or lower than 1 SD were removed from the original dataset.

```{r distrib-plot, fig.height=3, out.width="0.98\\columnwidth", fig.pos ="!h", fig.cap="(ref:distrib-plot)"}
data_dublin_geocoded %>%
  dplyr::filter(year == 2018) %>%
...
```{r osm-features-xgb, fig.cap="(ref:osm-features-xgb)", fig.height=7, fig.pos ="!h", out.width="0.98\\columnwidth"}
...
comparison_xgboost %>% 
  ...
  kableExtra::kable_styling(font_size = 8, latex_options = "HOLD_position") %>% 
  pack_rows(index = c("Tree based" = 4, "Linear regression based" = 4)) %>% 
  kableExtra::footnote("Missing value ares due to incompatibilities between data and methods.")
```
...
```{r census-features-xgb, fig.cap="(ref:census-features-xgb)", fig.height=7, fig.pos ="!h", out.width="0.98\\columnwidth"}
result_accuracy <- result %>%
  ggplot(aes(x = price, y = pred)) +
  geom_point(alpha = 0.1) +
  geom_smooth(
...
The authors would like to thank the developers of the following R packages used to process, analyse, display and report data: `r papaja::cite_r(file = "bib/r-references.bib")`.
